...
roles = ['All'] + sorted(df['Role'].unique())
selected_role = st.sidebar.selectbox("Role/Department", roles)

# Explode ethnicities for filtering
ethnicities = sorted(set(sum(df['Ethnicity'].dropna().str.split(','), [])))
ethnicities = ['All'] + [e.strip() for e in ethnicities]
...
col2.metric("Avg Recommendation", f"{filtered_df['Recommendation_Score'].mean():.1f}/10" if total>0 else "0")
...
col4.metric("Highly Fulfilled", f"{pct:.0f}%")

st.markdown("---")

# --- Crosstabs ---
recog_cross_role = pd.crosstab(filtered_df['Role'], filtered_df['Recognition_Short'], normalize='index')*100
growth_cross_role = pd.crosstab(filtered_df['Role'], filtered_df['Growth_Short'], normalize='index')*100

# Truncate role names for readability
...
fig_recog.update_layout(title="Recognition Sentiment by Role (%)", height=500, xaxis_title="Recognition Level", yaxis_title="Role")
...
fig_growth.update_layout(title="Growth Potential by Role (%)", height=500, xaxis_title="Growth Perception", yaxis_title="Role")
